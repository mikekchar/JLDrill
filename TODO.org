...
with traditional characters using CC-EDICT.
  ...
  :CurrentDate:  <2012-01-27 Fri>
  :EndDate: <2012-02-03 Fri>
  :PERCENT: 0
  :CurrentVelocity: 0
  :NeededVelocity: 0
  :END:
...
|-----------------+-------+------+---------+---------+--------|
| * Version 0.5.2 |    17 |   17 |   100.0 |    0.09 |   0.08 |
...
*** DONE Read Chinese collocations from Tatoeba example sentence
...
*** DONE Search dictionary for tratitional equivalent of simplified kanji
	When using the kanji popup, if you put the cursor over a simplified
	charater, no information is brought to the front.  Search the
	dictionary for words with the simplified character, if one is
	found, use the traditional kanji stored in the word as the kanji
	to search.
...
*** DONE Fix kanji listing in the edit search window
*** DONE Add Copyright information for Tatoeba database
*** DONE Remove Tanaka data files from distribution
*** DONE Fix Options picture
*** TODO Document Forgotten statistics
*** DONE Investigate Lintian failure (lintian bug???)
  After investigating, I'm pretty sure it's a lintian bug.
