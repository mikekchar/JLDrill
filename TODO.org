...
** TODO Schedule each problem type separately [0/3]
   ...
   both again.
   - [ ] Store a problem type in the item status rather than a
     schedule.  Store the schedule in the problem type
   - [ ] Allow more than one problem type in the status
   - [ ] Automatically create/remove statuses based on allowed problem
